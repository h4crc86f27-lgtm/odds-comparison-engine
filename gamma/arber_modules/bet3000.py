...
def do_bet3000_raw():
    ...
    cur  = conn.cursor()
    cur.execute("select * from raw_comps where site='bet3000'")
    rows = cur.fetchall()
    ids_done=[]
    for row in rows:
    ...
    #print("pulling bet3000 homepage")
    res = requests.get("https://bet500.bet/API/Countries/1/2/300")#,proxies = {"https":random.choice(proxies)})
    #soup = BeautifulSoup(res.text)
    data = res.json()
    #print("..souping..")
    print("countries:",len(data))
    for cd in data:
        country = cd['CategoryName']
        cuntid = cd['CategoryId']
        res = requests.get( "https://bet500.bet/API/TournamentsByCategoryId/1/" + str(cuntid) + "/2/300")#,timeout=10,proxies = {"https":random.choice(proxies)})
        country_data = res.json()
        for ccd in country_data:
            
            tid = ccd['TournamentId']
            comp_id = str(ccd['CategoryId']) + "|/" +str(tid)
            comp_name = ccd['TournamentName']
            if comp_id not in ids_done:

                x=cur.execute("insert into raw_comps(comp_name,comp_id,site,timestamp,country) values(%s,%s,%s,%s,%s)",(comp_name,comp_id,"bet3000",time.time(),country))
    #print("done bet3000 raw")
    conn.commit()
    conn.close()
...
def bet3000_data_thread(eventid,start_time,event_name):
    ...
    if data:
        for market in data:
            if market['GroupName']=="01|3 Way ":
                outcomes.append({"name":str(market['OutComeOrginal']).replace("1",home_team).replace("2",away_team),"price":float(market['OddValue'])})
            ...
            elif market['GroupName']=="10|Draw no Bet ":
                other_markets['DNB'][str(market['OutComeOrginal']).replace("1","Home").replace("2","Away")]=float(market['OddValue'])
                
            elif market['GroupName']=='02|Totals 2.5':
                other_markets['2.5'][market['OutComeOrginal']]=float(market['OddValue'])
    ...
    return {"id":eventid,"book":outcomes,"markets":extra_markets,"start_time":start_time}
...
def pull_data_bet3000(compid,league):#add comp_data as mutable param
    ...
    url="https://bet500.bet/API/FixtureMobile/1|/" + compid + "|/300/2/-600" ## << what is compid?
    ...
    if data:
        for match in data:
            e_list.append([match['MatchId'],match['TournamentDate'],match['EventName']])
            
    #print("b3000::matchcount::",(e_list))

    comp_data=[]

    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        future_to_url = {executor.submit(bet3000_data_thread, e[0],e[1],e[2]): e for e in e_list}
        for future in concurrent.futures.as_completed(future_to_url):
            #url = future_to_url[future]
            #print("inner sleep start..")
            #time.sleep(0.05)
            #print("inner sleep end..")
            try:
                data = future.result()
                comp_data.append(data)
            except Exception as exc:
                print('%r generated an exception: %s' % ("brr", exc))

    for cd in comp_data:
        try:
            do_odds_history_insert("bet3000", cd['id'], cd['book'], cd['markets'])
        except Exception as msg:
            pass#print("err on odds history insert..",str(msg))  
    #    print(">>",cd)
    #print("--END TOTO--")
    #with open("/home/arb_bot/beta_bet3000_fastbf/bet3000_dumps/" +  compid  + ".json","w") as f:
    #    f.write(json.dumps(comp_data))
    #print("bet3000:",len(comp_data))
    #print(comp_data)
    return comp_data
...
def insert_to_database_bet3000(t,b,league):
    ...
    #insert sectionbet3000_matches
    #print("TOTO###",b)
    odds_json={"bet3000_1_odds":toto_1_odds,"bet3000_x_odds":toto_x_odds,"bet3000_2_odds":toto_2_odds,"bet3000_under_2.5":toto_under_2p5,"bet3000_over_2.5":toto_over_2p5,"bet3000_under_3.5":toto_under_3p5,"bet3000_over_3.5":toto_over_3p5,"bet3000_dnb_home":toto_dnb_home,"bet3000_dnb_away":toto_dnb_away,
               "bet3000_dc_1x":toto_dc_1x,"bet3000_dc_12":toto_dc_12,"bet3000_dc_x2":toto_dc_x2,
           "bet3000_1_ht":toto_1_ht,"bet3000_x_ht":toto_x_ht,"bet3000_2_ht":toto_2_ht,
    ...
    cur.execute("select * from bet3000_matches where bet3000_event_id=%s and betfair_event_id=%s",(t['event_id'],b['event_id']))
    rows = cur.fetchall()
    if len(rows)>0:
        #print("found event,, updating",t['event_id'],b['event_id'])
        cur.execute("update bet3000_matches set timestamp=%s,bet3000_data=%s,t1_bet3000_fuzzy=%s,t2_bet3000_fuzzy=%s where bet3000_event_id=%s and betfair_event_id=%s",
        ...
        cur.execute("insert into bet3000_matches (timestamp,bet3000_event_id,betfair_event_id,team_1_bet3000,team_2_bet3000,team_1_betfair,team_2_betfair,bet3000_data,t1_bet3000_fuzzy,t2_bet3000_fuzzy,ignored,league) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
        ...
        for rd in ref_data:#this is a list not the dict version
            ...
            if abs(convert_string_timestamp(bd['start_time'])- convert_string_timestamp(rd['start_time']))<=0:#within 30s of each other..
                ...
                if len(set(book_names_lower).intersection(set(rd_teams)))>0:# in rd_teams or bd['book'][1]['name'].lower() in rd_teams or bd['book'][2]['name'].lower() in rd_teams:
                    ...
                    insert_to_database_bet3000(tt,rd,league)
