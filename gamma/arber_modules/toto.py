...
def get_extra_markets_toto(ld):
    ...
    res = requests.get(url,timeout=timeout_count)#,proxies = {"https":random.choice(proxies)})
    colls = res.json()['data']['events'][0]['collections'] ## << this is broken,, so where do i get the marketids?
    market_ids=[]
    for c in colls:
        #if c['name']=='Wedstrijd' or c['name']=='Doelpunten' or c['name']=="Populair" or c['name']=="Helft":#dnb/3.5
        #    for m in c['marketIds']:
        #        market_ids.append(m)
        if c['name'] in ['Wedstrijd', 'Doelpunten', 'Populair', 'Helft']:
            market_ids.extend(c['marketIds'])
    ...
    try:
        ...
        headers={'user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.100 Safari/537.36'}
        res = requests.get(url,timeout=timeout_count,headers=headers)#,proxies = {"https":random.choice(proxies)})
        data = res.json()
...
def toto_data_thread(event_outcome):
            f,market = event_outcome

            print("totomatch:",f['id'],market)
            outcomes=[]
            extra_markets=[]
            markets = f['markets']
            with open("/home/arb_bot/gamma/toto_dumps/" + str(f['id']) + "_markets.json","w") as f:
                x=f.write(json.dumps(markets))
            ...
            try:
                if market:
                    ...
                    for outcome in market['outcomes']:
                        if outcome['subType']=='H':
                            home_outcome= outcome['prices'][0]['decimal']
                            home = outcome['name']
                        elif outcome['subType']=='A':
                            away_outcome= outcome['prices'][0]['decimal']
                            away = outcome['name']
                        else:
                            draw_outcome= outcome['prices'][0]['decimal']
                            draw = outcome['name']
                ...
                pass#print("err getting MR outcomes",str(msg))
            ...
            if 1:#try:
                ...
                pass#print("err in other markets",str(msg))#print("extra markets >> ")
...
def pull_data_toto(compid,league):#add comp_data as mutable param
    print("TOTO DATA >>>",compid,league)
    dates=[]
    for dta in range(-1,10):
        dates.append(str(datetime.datetime.now()+datetime.timedelta(days=dta))[0:10])
    url="https://content.toto.nl/content-service/api/v1/q/time-band-event-list?maxMarkets=20&marketSortsIncluded=--%2CHH%2CHL%2CMR%2CWH&marketGroupTypesIncluded=MONEYLINE%2CROLLING_SPREAD%2CROLLING_TOTAL%2CSTATIC_SPREAD%2CSTATIC_TOTAL&allowedEventSorts=MTCH&includeChildMarkets=true&prioritisePrimaryMarkets=true&includeCommentary=true&includeMedia=true&drilldownTagIds=" + compid + "&maxTotalItems=300&maxEventsPerCompetition=100&maxCompetitionsPerSportPerBand=3&maxEventsForNextToGo=5&startTimeOffsetForNextToGo=600&dates=" + dates[0] + "T22%3A00%3A00Z%2C" + dates[1] + "T22%3A00%3A00Z%2C" + dates[2] + "T22%3A00%3A00Z%2C" + dates[3] + "T22%3A00%3A00Z%2C" + dates[4] + "T22%3A00%3A00Z%2C" + dates[5] + "T22%3A00%3A00Z%2C" + dates[6] + "T22%3A00%3A00Z%2C" + dates[7] + "T22%3A00%3A00Z%2C" + dates[8] + "T22%3A00%3A00Z%2C" + dates[9] + "T22%3A00%3A00Z"#%2C" + dates[10] + "T22%3A00%3A00Z%2C" + dates[11] + "T22%3A00%3A00Z%2C" + dates[12] + "T22%3A00%3A00Z%2C" + dates[13] + "T22%3A00%3A00Z"
    
    for counter in range(1):
        try:
            headers={'user-agent':'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.4844.83 Safari/537.36'}
            res = requests.get(url,timeout=timeout_count,headers=headers)#,proxies = {"https":random.choice(proxies)})
    ...
    #with open("/home/arb_bot/gamma/toto_dumps/comp_" + str(compid) + ".json","w") as f:
    #    x=f.write(json.dumps(data))

    #here,, call for a different chunk of data, make a dict,, and then feed it into the e_list..
    #so that i can pull out the MR outcomes as per usual..
    edict={}
    try:
        url="https://content.toto.nl/content-service/api/v1/q/event-list?startTimeFrom=" + dates[0] + "T22%3A00%3A00Z&startTimeTo=" + dates[9] + "T21%3A59%3A59Z&maxEvents=100&popularityLimit=5&orderEventsBy=popularity&maxMarkets=10&orderMarketsBy=displayOrder&marketSortsIncluded=--%2CCS%2CDC%2CDN%2CHH%2CHL%2CMH%2CMR%2CWH&marketGroupTypesIncluded=CUSTOM_GROUP%2CMONEYLINE%2CROLLING_SPREAD%2CROLLING_TOTAL%2CSTATIC_SPREAD%2CSTATIC_TOTAL&eventSortsIncluded=MTCH&includeChildMarkets=true&prioritisePrimaryMarkets=true&includeCommentary=true&includeMedia=true&drilldownTagIds=" + compid

        res = requests.get(url,timeout=timeout_count)
        other_data = res.json()
        
        events = other_data['data']['events']
        for event in events:
            eid = event['id']
            markets = event['markets']
            outcome_data=None
            for market in markets:
                if market['subType']=='MR':
                    outcome_data = market
                    break
            edict[eid]=outcome_data
    except Exception as msg:
        pass#print("failed extra data thing..",str(msg))
    ...
    for e in events_base:
        for f in e['events']:
            if f['id'] in edict:
                #print("id found in edict!!")
                #ime.sleep(2)
                outcomes=edict[f['id']]
                #print(f['id'])

                #for outcome in edict[f['id']]['outcomes']:
                #    print(outcome['name'],outcome['prices'][0]['decimal'])
                #time.sleep(3)
            else:
                outcomes = None
            e_list.append([f,outcomes])
    ...
    #    print(">>",cd)
    #print("--END TOTO--")
    #with open("/home/arb_bot/gamma/toto_dumps/" +  compid  + ".json","w") as f:
...
def insert_to_database_toto(t,b,league):
    ...
    cur.execute("select 1 from toto_matches where toto_event_id=%s and betfair_event_id=%s",(t['event_id'],b['event_id']))
    row = cur.fetchone() # was rows = cur.fetchall()
    #if len(rows)>0:
    if row is not None:
        ...
        #print("updated:",t['event_id'],b['event_id'],odds_json)
    else:
        ...
        for rd in ref_data:#this is a list not the dict version
            ...
            if abs(convert_string_timestamp(bd['start_time'])- convert_string_timestamp(rd['start_time']))<=0:#within 30s of each other..
                ...
                if len(set(book_names_lower).intersection(set(rd_teams)))>0:# in rd_teams or bd['book'][1]['name'].lower() in rd_teams or bd['book'][2]['name'].lower() in rd_teams:
                    ...
                    insert_to_database_toto(tt,rd,league)
