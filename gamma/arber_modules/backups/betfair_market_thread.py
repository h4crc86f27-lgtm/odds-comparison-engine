...
def betfair_1x2_thread(comp_market,event_id,retval):
        ...
        try:
            ...
            retval['odds'].append({"last_back_price":0,"last_back_vol":0,"lowest_back_price":0,"lowest_back_vol":0,"lay_price":0,"lay_vol":0})
            #print(str(msg),"H2H",event_id,which_market)
        #return retval

def betfair_uo25_thread(comp_market,event_id,retval):
...
def betfair_uo35_thread(comp_market,event_id,retval):
	#uo 3.5
...
def betfair_dnb_thread(comp_market,event_id,retval):
...
def betfair_dc_thread(comp_market,event_id,retval):
        #print("----",event_id,"Getting DNB----")
...
def betfair_ht_thread(comp_market,event_id,retval):
        #print("----",event_id,"Getting DNB----")
...
def betfair_data_thread(event_id,event):
        ...
        t1=Thread(target=betfair_1x2_thread,args = (comp_market,event_id,retval,))
    ####### here do the other markets,, will clean this up later.
        #uo 2.5
        t2=Thread(target=betfair_uo25_thread,args = (comp_market,event_id,retval,))
        #uo3.5
        t3=Thread(target=betfair_uo35_thread,args = (comp_market,event_id,retval,))
        #DNB
        t4=Thread(target=betfair_dnb_thread,args = (comp_market,event_id,retval,))
        #DC
        t5=Thread(target=betfair_dc_thread,args=(comp_market,event_id,retval,))
        #1x2_HT
        t6=Thread(target=betfair_ht_thread,args = (comp_market,event_id,retval,))
        t1.start()
        t2.start()
        t3.start()
        t4.start()
...
def pull_betfair_data(compid,league):
    ...
    t_count = thread_count#min(len(e_list),thread_count)

    with concurrent.futures.ThreadPoolExecutor(max_workers=len(e_list)) as executor:
