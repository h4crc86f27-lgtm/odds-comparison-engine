...
def pull_extra_markets_contra(eventid):# = "5316624"):
    ...
    res = requests.post("https://s.linex11.com/api/front/events/details",
                        data={"EventId":str(eventid)},proxies = {"https":random.choice(proxies)},timeout=3)
    data = res.json()
...
def do_contra_raw():
    ...
    for p in range(0,5):
        ...
        res=requests.post("https://s.linex11.com/api/front/events",data=payload,proxies = {"https":random.choice(proxies)},timeout=3)
        data = res.json()
        for d in data:
...
def pull_data_contra(compid,league):#country,tourny):
    ...
    for counter in range(3):
        try:
            ...
            pass#print("contra comp err,, retrying")
