...
datetime_in_a_week = (datetime.datetime.utcnow() + datetime.timedelta(weeks=7)).strftime("%Y-%m-%dT%TZ")
...
def process_event_odds(event_id,event_data):
    ...
    try:
        ...
        for runner in match_book:
            ...
            last_back_vol = 0
            #print("---")#,runner.name)
            lowest_back_price = runner.last_price_traded
            vwap = 0
            total_vol =0
            ...
            retval['odds'].append({"last_back_price":last_back_price,"last_back_vol":last_back_vol,"lowest_back_price":lowest_back_price,"lowest_back_vol":0,"lay_price":lay_price,"lay_vol":lay_volume})  
    ...
    try:
        ...
        for runner in books:
            ...
            try:
                ...
                lay_volume_ou25 = 0
            retval['vwaps'][which_market]=round(vwap,2)
            retval['markets'].append({"last_back_price_"+which_market:last_back_price_ou25,"last_back_vol_"+which_market:last_back_vol_ou25,"lowest_back_price_"+which_market:lowest_back_price,"lowest_back_vol_"+which_market:0,"lay_price_"+which_market:lay_price_ou25,"lay_vol_"+which_market:lay_volume_ou25})
    except Exception as msg:
        which_market="Under_2.5"
        ...
        if 1:#len(books)>0:
            ...
            for runner in books:
                ...
                try:
                    ...
                    lay_volume_dnb= 0
                retval['vwaps'][which_market]=round(vwap,2)
                retval['markets'].append({"last_back_price_"+which_market:last_back_price_dnb,"last_back_vol_"+which_market:last_back_vol_dnb,"lowest_back_price_"+which_market:lowest_back_price,"lowest_back_vol_"+which_market:0,"lay_price_"+which_market:lay_price_dnb,"lay_vol_"+which_market:lay_volume_dnb})
...
def pull_type_events(compid=7):#horse racing
    filter = {"eventTypeIds":[str(compid)]}
    events = trading.betting.list_events(filter=filter)
    retval=[]
    for event in events:
        if event.event.name.find("20th Jun")>-1:
            if event.event.country_code in ["AU","NZ"]:
                retval.append(int(event.event.id))
    return retval
...
def pull_all_events(compids):
    ...
    filter={"competitionIds":compids,'marketStartTime': {'from': start_date_str,'to': end_date_str}}
...
def pull_betfair_data(compid,league,betfair_markets,betfair_data_dict,events=[],foobar=None):
    ...
    for e in eventids:
        ...
        try:
            ...
            try:
                if "toto" not in sys.argv and "live90bet" not in sys.argv:
    ...
    try:
        if "toto" not in sys.argv and "live90bet" not in sys.argv:
            with open("/home/arb_bot/gamma/bf_dumps/" + league + "_" + str(compid) + ".json","w") as f:
