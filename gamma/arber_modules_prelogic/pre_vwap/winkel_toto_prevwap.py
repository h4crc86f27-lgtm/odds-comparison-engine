...
def winkel_toto_data_thread(e):
    ...
    #print("winkelurl:",e)
    res = requests.post(e,headers={"content-type":"application/json","accept-encoding":"gzip, deflate, br"},proxies = {"https":random.choice(proxies)},timeout=3)
...
def pull_data_winkel_toto(compid,league):#country,tourny):
    ...
    res = requests.post(url,headers={"content-type":"application/json","accept-encoding":"gzip, deflate, br"},proxies = {"https":random.choice(proxies)},timeout=3)
    ...
    for cd in comp_data:
        try:
            ...
            print("WINKEL err on odds history insert..",str(msg))   
