...
def dump():
    ...
    for event in events:
        try:
            ...
            pass
    #dump
    with open("/var/www/vhosts/default/htdocs/dump.pkl","wb") as f:
        pickle.dump(out,f)
    ...
    with open("/var/www/vhosts/default/htdocs/dmp.pkl","wb") as f:
    ...
    if 1:
        ...
        with open("/var/www/vhosts/default/htdocs/dmp_plc.pkl","wb") as f:
            pickle.dump(market_dict,f)
    #wim
    """mens={}
    mens_events=[]
    id_dict={}
    filter = {"competitionIds":[str(12592395)]}
    ...
    with open("/var/www/vhosts/default/htdocs/menwim.pkl","wb") as f:
        pickle.dump(mens,f)
    #fems=[]
    womens={}
    womens_events=[]
    id_dict={}
    filter = {"competitionIds":[str(12592725)]}
    events = trading.betting.list_events(filter=filter)
    for event in events:
        if event.event.name.find(" v ")>-1:
    ...
    #fems=[]
    with open("/var/www/vhosts/default/htdocs/womenwim.pkl","wb") as f:
...
while 1:#isnt_running:
    try:
        ...
        if book_list == ['bet3000'] or book_list ==['yess365']:
            try:
